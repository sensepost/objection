...
class AndroidGadget(BasePlatformGadget):
    ...
    def set_architecture(self, architecture: str):
        ...
        if architecture not in self.architectures.keys():
            raise Exception(
                "Invalid architecture `{0}` set. Valid options are: {1}".format(
    ...
    def download(self):
        ...
        with open(library_destination, "wb") as f:
            click.secho(
                "Downloading {0} library to {1}...".format(
                    self.architecture, library_destination
                ),
                fg="green",
                dim=True,
            )
    ...
    def unpack(self):
        ...
        click.secho(
            "Unpacking {0}...".format(self.get_frida_library_path(packed=True)),
            dim=True,
        )
...
class AndroidPatcher(BasePlatformPatcher):
    ...
    required_commands = {
        ...
        "apktool": {"installation": "apt install apktool (Kali Linux)"},
    ...
    def __init__(
        ...
        self.apk_temp_frida_patched_aligned = (
            self.apk_temp_directory + ".aligned.objection.apk"
        )
        self.aapt = None
        ...
        self.keystore = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "../assets", "objection.jks"
        )
        self.netsec_config = os.path.join(
    ...
    def is_apktool_ready(self) -> bool:
        ...
        click.secho("Detected apktool version as: " + cmd_out, dim=True)

        # ensure we have at least apktool MIN_VERSION
        if semver.compare(cmd_out, min_version) < 0:
            click.secho(
                "apktool version should be at least " + min_version, fg="red", bold=True
            )
            click.secho(
                "Please see the following URL for more information: "
                "https://github.com/sensepost/objection/wiki/Apktool-Upgrades",
                fg="yellow",
            )
            return False
    ...
    def _get_android_manifest(self) -> ElementTree.ElementTree:
        ...
        if self.skip_resources is True and self.manifest is None:
            click.secho(
                "Cannot manually parse the AndroidManifest.xml when --skip-resources "
                "is set, remove this and try again, or manually specify a manifest with --manifest.",
                fg="red",
            )
            raise Exception(
                "Cannot --skip-resources when trying to manually parse the AndroidManifest.xml"
            )
        ...
        if self.manifest is not None:
            ...
            return ElementTree.parse(
                os.path.join(self.apk_temp_directory, "AndroidManifest.xml")
            )
    ...
    def _get_launchable_activity(self) -> str:
        ...
        click.secho(
            (
                "Unable to determine the launchable activity using aapt, trying "
                "to manually parse the AndroidManifest for activity aliases..."
            ),
            dim=True,
            fg="yellow",
        )
        ...
        # activity is
        click.secho(
            "Unable to determine the launchable activity for this app.", fg="red"
        )
    ...
    def unpack_apk(self):
        ...
        if len(o.err) > 0:
            click.secho(
                "An error may have occurred while extracting the APK.", fg="red"
            )
            click.secho(o.err, fg="red")
    ...
    def add_network_security_config(self):
        ...
        shutil.copyfile(
            self.netsec_config,
            os.path.join(sec_config_path, "network_security_config.xml"),
        )
    ...
    def _determine_smali_path_for_class(self, target_class) -> str:
        ...
        activity_path = (
            os.path.join(self.apk_temp_directory, "smali", target_class) + ".smali"
        )
        ...
        if not os.path.exists(activity_path):
            ...
            for x in range(2, 100):
                smali_path = os.path.join(
                    self.apk_temp_directory, "smali_classes{0}".format(x)
                )
    ...
    def _determine_end_of_smali_method_from_line(smali: list, start: int) -> int:
        ...
        end_methods = [
            (i + start) for i, x in enumerate(smali[start:]) if ".end method" in x
        ]
    ...
    @staticmethod
    def _determine_first_inject_point_of_smali_method_from_line(
        smali: list, start: int
    ) -> int:
        """
    ...
    def _patch_smali_with_load_library(
        self, smali_lines: list, inject_marker: int
    ) -> list:
        """
        ...
        if "clinit" in smali_lines[inject_marker]:
            ...
            inject_point = self._determine_first_inject_point_of_smali_method_from_line(
                smali_lines, inject_marker
            )
            click.secho(
                "Injecting loadLibrary call at line: {0}".format(inject_point),
                dim=True,
                fg="green",
            )
            ...
            click.secho(
                "Injecting loadLibrary call at line: {0}".format(inject_marker),
                dim=True,
                fg="green",
            )
    ...
    def _revalue_locals_count(self, patched_smali: list, inject_marker: int):
        ...
        def _h():
            click.secho(
                "Could not update .locals value. Sometimes this may break things,"
                "but not always. If the applications crashes after patching, try "
                "and add the --pause flag, fixing the patched smali manually.",
                fg="yellow",
            )
        ...
        end_of_method = self._determine_end_of_smali_method_from_line(
            patched_smali, inject_marker
        )
        ...
        defined_locals = [
            i
            for i, x in enumerate(patched_smali[inject_marker:end_of_method])
            if ".locals" in x
        ]

        if len(defined_locals) <= 0:
            click.secho(
                "Unable to determine any .locals for the target constructor",
                fg="yellow",
            )
            _h()
            return patched_smali
        ...
        try:
            ...
            click.secho("Unable to parse .locals value for the injected constructor with error: {0}".format(str(e)),
                fg="yellow",
                )
            _h()
        ...
        click.secho(
            "Current locals value is {0}, updating to {1}:".format(
                defined_local_value_as_int, new_locals_value
            ),
            dim=True,
        )
    ...
    def inject_load_library(self, target_class: str = None):
        ...
        if target_class:
            click.secho(
                "Using target class: {0} for patch".format(target_class),
                fg="green",
                bold=True,
            )
        else:
        ...
        inject_marker = [
            i for i, x in enumerate(smali_lines) if "# direct methods" in x
        ]
        ...
        click.secho(
            "Writing patched smali back to: {0}".format(activity_path), dim=True
        )

        with open(activity_path, "w") as f:
            f.write("".join(patched_smali))

    def add_gadget_to_apk(
        self, architecture: str, gadget_source: str, gadget_config: str
    ):
        """
        ...
        if gadget_config:
            ...
            shutil.copyfile(
                gadget_config, os.path.join(libs_path, "libfrida-gadget.config.so")
            )
    ...
    def build_new_apk(self, use_aapt2: bool = False):
        ...
        click.secho(
            "Rebuilding the APK with the frida-gadget loaded...", fg="green", dim=True
        )
        cmd_out = delegator.run(
            self.list2cmdline(
                [
                ...
                + ["-o", self.apk_temp_frida_patched]
            ),
            timeout=self.command_run_timeout,
        )

        if len(cmd_out.err) > 0:
            click.secho(
                (
                    "Rebuilding the APK may have failed. Read the following "
                    "output to determine if apktool actually had an error: \n"
                ),
                fg="red",
            )
            click.secho(cmd_out.err, fg="red")

        click.secho(
            "Built new APK with injected loadLibrary and frida-gadget", fg="green"
        )
    ...
    def zipalign_apk(self):
        ...
        cmd_out = delegator.run(
            self.list2cmdline([
                ...
                if os.path.exists(self.apk_temp_frida_patched)
                else self.apk_source,
                self.apk_temp_frida_patched_aligned,
            ])
        )
        ...
        if len(cmd_out.err) > 0:
            click.secho(
                (
                    "Zipaligning the APK may have failed. Read the following "
    ...
    def sign_apk(self):
        ...
        if len(cmd_out.err) > 0:
            ...
            click.secho(cmd_out.out, fg="yellow")
            click.secho(cmd_out.err, fg="red")
    ...
    def __del__(self):
        ...
        try:
            ...
            click.secho(
                "Failed to cleanup with error: {0}".format(err), fg="red", dim=True
            )
