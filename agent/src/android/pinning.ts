...
const okHttp3CertificatePinnerCheckOkHttp = (ident: string): any | undefined => {
  ...
  return wrapJavaPerform(() => {
    try {
      const certificatePinnerCheckOkHttp = Java.use("okhttp3.CertificatePinner");
      certificatePinnerCheckOkHttp["check$okhttp"].overloads.forEach((overload) => {
          // get the argument types for this overload
          var calleeArgTypes = overload.argumentTypes.map((arg) => arg.className);
